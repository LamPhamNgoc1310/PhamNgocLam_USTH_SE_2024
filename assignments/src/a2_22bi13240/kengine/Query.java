...
public class Query {
	...
	public void addKey(String w) throws NotPossibleException {
		if (matches.isEmpty() || keys.contains(w))
			throw new NotPossibleException("Query.addKey: query is empty OR keyword already exists in query: " + w);
	 ...
	 * @param h a <code>Hashtable</code> that maps interesting words in
	 ...
	 *          keyword entries in <code>h</code> to <code>matches</code> as a query
	 *          result, else does nothing
	 * 
	 * @version 4.0
	 */
	 ...
	 * @effects if <code>0 <= i < size</code> then returns the ith matching document
	 *          in <code>matches</code>, else throws
	...
	public Doc fetch(int i) throws IndexOutOfBoundsException {
		if (0 <= i && i < size()) {
			...
			throw new IndexOutOfBoundsException("Query.fetch: document index is invalid " + i);
	}
	...
	public Iterator<Doc> matchIterator() {
		if (matches.isEmpty()) {
			return null;
		}
