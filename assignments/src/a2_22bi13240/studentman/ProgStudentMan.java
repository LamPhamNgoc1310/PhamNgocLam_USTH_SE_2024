...
public class ProgStudentMan {
  ...
  public ProgStudentMan() {
	  try {
		 ...
		 System.out.println("Your object hasn't been initialised");
	  }
  }
  ...
  public Query search(String[] words) throws NotPossibleException{
		  ...
		  if (words == null || words.length == 0) {
			  throw new NotPossibleException("ProgStudentMan.search: Your keywords does not exist or might be wrong");
		  }
		  Query queryResult = engine.queryFirst(words[0]);
		  for(int i = 1; i < words.length; i++) {
			  queryResult = engine.queryMore(words[i]);
		  }
		  return queryResult;
